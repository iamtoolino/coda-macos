...
enum SelfTests {
  ...
  static func run() -> Bool {
    ...
    if failures.isEmpty {
      print("Coda playback spike self-tests passed (30 checks).")
      return true
    }
  ...
  static func runMPVLocalTransitionTest() async -> Bool {
    ...
    for _ in 0..<80 {
      ...
      if player.currentIndex == 1 {
        player.clear()
        print("mpv advanced through a local two-item playlist successfully.")
        return true
      }
    ...
    player.clear()
    print("FAILED: mpv did not advance through the local two-item playlist.")
    return false
  }
